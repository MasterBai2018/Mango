...
from src.utils.common import is_digit, wait_gdb_attach
...
class NissanAIBSClient(AudioDataMixin):
    """AIBS客户端封装"""
    # 类变量：所有实例共享的时间戳和音频进度
    ...
    abstime = 1         # 共享抽象时间戳
    dirCallbackType = None
    dirCallbacketId = None
    m_switch_lang_finish_event = threading.Event()
    ...
    def __init__(self, client_name: str, lib_path: str, config=None):
        ...
        # 初始化共享变量（仅在第一次实例化时）
        with NissanAIBSClient._shared_lock:
            if NissanAIBSClient.m_timestamp is None:
                NissanAIBSClient.m_timestamp = get_timestamp()
                NissanAIBSClient.audio_progress = 0
                NissanAIBSClient.audio_path = "NULL"
                NissanAIBSClient.abstime = 1
    ...
    def _callback_wrapper(self, status, result):
        ...
        try:
            ...
            if callback_func:
                try:
                    callback_data['audiotime'] = str(round(NissanAIBSClient.audio_progress/1000, 2))
                    callback_data['audio'] = NissanAIBSClient.audio_path
    ...
    def _process_pcm_chunk(self, data_chunk: bytes, delta_ms: int):
        """
        发送单包PCM数据并推进Nissan AIBS共享时间戳
        """
        with NissanAIBSClient._shared_lock:
            current_timestamp = NissanAIBSClient.m_timestamp if NissanAIBSClient.abstime == 1 else NissanAIBSClient.audio_progress
            ...
            NissanAIBSClient.audio_progress += delta_ms
            NissanAIBSClient.m_timestamp += delta_ms
    ...
    def data(self, command: DSLCommand) -> int:
        ...
        try:
            ...
            for param in command.params[1:]:
                if '=' in param:
                    ...
                    if key == 'frame':
                        ...
                        NissanAIBSClient.abstime = int(value)
                    elif key == 'range':
            ...
            NissanAIBSClient.audio_path = audio_path
            ...
            with NissanAIBSClient._shared_lock:
                wall = get_timestamp()
                stream_end = NissanAIBSClient.m_timestamp
                NissanAIBSClient.m_timestamp = max(wall, stream_end)
                os.environ["AUDIO_BASE_TIMESTAMP"] = str(NissanAIBSClient.m_timestamp)
    ...
    def set_vr_event(self, command: DSLCommand) -> int:
        ...
        try:
            ...
            event_data_json.update({
                "et_id": uuid(),
                "mts": NissanAIBSClient.m_timestamp
            })
